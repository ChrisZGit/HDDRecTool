...
int FileHandler::estimateStripeSize()
{
	...
	int stripeSize = -1;
	int counters[11]={};
	for (unsigned int bl = 0; bl < 11; ++bl)
	{
	...
	//			size_t currentEdge = results.at(i).at(pointer[i]).second;
				if (results.at(i).at(pointer[i]).second > 5.5f)
				{
		...
		for (size_t i = 0; i < inFiles.size(); ++i)
		{
			inFiles.at(i)->setBlockSize(inFiles.at(i)->getBlockSize()*2);
		}
	}
	int x = 0;
	int value[11] = {1024, 512, 256, 128, 64, 32, 16, 8, 4, 2, 1};
	std::cout << "Found:\n";
	...
	int tmp1 = counters[0], tmp2 = 0;
	for (int i = 1; i < 11; i++)
	{
		if(2*counters[i-1] < counters[i] && counters[i] > 0)
		{
			tmp1 = counters[i];
			tmp2 = value[i];
		}
